...
def analyze(stock_codes: List[str], period: str, interval: str, predict_days: int, analysis_type: str):
    ...
    try:
        ...
        # 获取股票数据
        click.echo("\n[2/2] 获取股票数据...")
        results = cli.fetch(stock_codes=stock_codes, period=period, interval=interval)
        ...
        for idx, stock_data in enumerate(results, 1):
            ...
            if analysis_type in ['all', 'predict']:
                click.echo("\n🔮 执行预测分析...")
                click.echo("- 正在加载预测模型...")
                cli.predict(stock_code=stock_code, days=predict_days)
                click.echo("✅ 预测分析完成")
                
            if analysis_type in ['all', 'turtle']:    
                click.echo("\n🐢 执行海龟策略分析...")
                click.echo("- 计算技术指标...")
                cli.analyze_turtle(stock_code=stock_code)
                click.echo("✅ 海龟策略分析完成")
                
            if analysis_type in ['all', 'sentiment']:
                click.echo("\n😊 执行情绪分析...")
                click.echo("- 获取市场情绪数据...")
                cli.analyze_sentiment(stock_code=stock_code)
                click.echo("✅ 情绪分析完成")
                
            if analysis_type in ['all', 'ai']:
                click.echo("\n🤖 执行AI分析...")
                click.echo("- 正在进行深度学习分析...")
                cli.analyze_ai(stock_code=stock_code)
                click.echo("✅ AI分析完成")
                
            if analysis_type == 'all':
                click.echo("\n📊 执行全面分析...")
                click.echo("- 整合所有分析结果...")
                cli.full_analysis(stock_code=stock_code)
                click.echo("✅ 全面分析完成")
